...
export default function StyleEditorPage({ params }: { params: { id: string } }) {
        ...
        <div>
          ...
          <div className="space-y-2 mb-3">
            {pros.map((pro, i) => (
              <div key={i} className="flex items-center gap-2 bg-green-50 border border-green-100 rounded-lg px-3 py-2.5">
                ...
                <button
                  onClick={() => setPros(prev => prev.filter((_, j) => j !== i))}
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="장점 입력 후 Enter..."
              className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-400 transition-colors"
            />
            <button onClick={addPro} className="bg-green-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-green-700 transition-colors">추가</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2 mb-3">
            {cons.map((con, i) => (
              <div key={i} className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-lg px-3 py-2.5">
                ...
                <button
                  onClick={() => setCons(prev => prev.filter((_, j) => j !== i))}
                  className="text-gray-300 hover:text-red-400 text-lg leading-none transition-colors"
                >×</button>
              </div>
            ))}
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="단점 입력 후 Enter..."
              className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400 transition-colors"
            />
            <button onClick={addCon} className="bg-red-500 text-white text-sm px-4 py-2 rounded-lg hover:bg-red-600 transition-colors">추가</button>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="text-sm font-medium text-gray-700">
              사진 <span className="font-normal text-gray-400">({style.photos?.length ?? 0}장)</span>
            </label>
            <button
          ...
          {(!style.photos || style.photos.length === 0) ? (
            ...
            >
              ...
              <div className="text-gray-300 text-xs mt-1">여러 장 동시 선택 가능</div>
            </div>
          ) : (
              ...
              {style.photos.map((photo, i) => (
                <div key={photo.id} className="relative group aspect-square">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 bg-black/60 text-white w-6 h-6 rounded-full text-xs opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center hover:bg-black/80"
                  >×</button>
                </div>
              ))}
              <div
                onClick={() => fileInputRef.current?.click()}
                className="aspect-square border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center cursor-pointer hover:border-gray-300 hover:bg-gray-50 transition-all"
              >
